...
class AdminVerClasesFragment : Fragment(), AdminVerClasesAdapter.AdminSelClass {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        botClasesAlumnos.setOnClickListener {

                Toast.makeText(context, "NO IMPLEMENTADO POR EL MOMENTO", Toast.LENGTH_LONG).show()
            findNavController().navigate(R.id.action_adminClaseCnAlumnosFragment_to_adminClassAlumFragment)
        }
    ...
    override fun showDialog(adminClase: ClaseEntity) {
        Log.d("trae clase", adminClase.toString())
        ...
        val managerDialog: AlertDialog? = activity?.let {
            ...
            builder.apply {
                setPositiveButton(
                    R.string.modificar_clase,
                    DialogInterface.OnClickListener { dialog, id ->

                        myViewModel.deleteClase(claseToDelete)
                        myViewModel.clasetoModify(adminClase)
